...
async fn main() {
    ...
    if create_index {
        ...
        let mut dataset = Dataset::write(batches, "./full_text_search_example", None)
            .await
            .unwrap();
    ...
    let dataset = Dataset::open("./full_text_search_example").await.unwrap();
    let query = tokens[0];
    println!("query: {:?}", query);
    let batch = dataset
        .scan()
        .full_text_search(FullTextSearchQuery::new("doc".to_owned(), query.to_owned()))
        .unwrap()
        .try_into_batch()
        .await
        .unwrap();
