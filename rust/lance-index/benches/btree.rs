...
fn bench_btree_search(c: &mut Criterion) {
    ...
    let data = gen_int32_random(TOTAL, BATCH_SIZE);
    let store = store.clone();
    // Train the B-tree index (not benchmarked)
    rt.block_on(async {
        let sub_index_trainer = FlatIndexMetadata::new(data.schema().field(0).data_type().clone());
    ...
    c.bench_function(format!("btree_equals_({TOTAL})").as_str(), |b| {
        b.to_async(&rt).iter(|| async {
            let query_value = SargableQuery::Equals(ScalarValue::Int32(Some(500000)));
            index
    ...
    c.bench_function(format!("btree_range_({TOTAL})").as_str(), |b| {
        b.to_async(&rt).iter(|| async {
            let query_range = SargableQuery::Range(
    ...
    c.bench_function(format!("btree_is_in_({TOTAL})").as_str(), |b| {
        b.to_async(&rt).iter(|| async {
            let query_values = SargableQuery::IsIn(vec![
                ...
                ScalarValue::Int32(Some(600000)),
                ScalarValue::Int32(Some(700000)),
                ScalarValue::Int32(Some(800000)),
                ScalarValue::Int32(Some(900000)),
                ScalarValue::Int32(Some(1000000)),
            ]);
    ...
    c.bench_function(format!("btree_is_null_({TOTAL})").as_str(), |b| {
        b.to_async(&rt).iter(|| async {
