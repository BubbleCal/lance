...
lazy_static! {
    pub static ref TOKENIZER: tantivy::tokenizer::TextAnalyzer = {
        ...
        let stemmer = tantivy::tokenizer::Stemmer::new(tantivy::tokenizer::Language::English);
        let lower_case = tantivy::tokenizer::LowerCaser {};
        let tokenizer = tantivy::tokenizer::SimpleTokenizer::default();
    ...
    async fn search(&self, query: &dyn AnyQuery) -> Result<RowIdTreeMap> {
        ...
        let row_ids = match query {
            SargableQuery::FullTextSearch(query) => {
                ...
                self.bm25_search(token_ids, query.limit)
                    .into_iter()
                    .map(|(row_id, _)| row_id)
            }
            query => {
    ...
    async fn test_inverted_index() {
        ...
        let row_ids = invert_index
            .search(&super::SargableQuery::FullTextSearch(
                FullTextSearchQuery::new("doc".to_owned(), "lance".to_owned()),
            ))
            .await
            .unwrap();
        ...
        let row_ids = invert_index
            .search(&super::SargableQuery::FullTextSearch(
                FullTextSearchQuery::new("doc".to_owned(), "database".to_owned()),
            ))
            .await
            .unwrap();
        assert_eq!(row_ids.len(), Some(3));
