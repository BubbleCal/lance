...
impl Dataset {
    ...
    fn scanner(
        ...
        if let Some(full_text_query) = full_text_query {
            let column = full_text_query
                .get_item("column")?
                .ok_or_else(|| PyKeyError::new_err("Need column for full text search"))?
                .to_string();
            let query = full_text_query
                .get_item("query")?
                .ok_or_else(|| PyKeyError::new_err("Need query for full text search"))?
                .to_string();
            let limit = full_text_query
                .get_item("limit")?
                .and_then(|l| {
                    if l.is_none() {
                        None
                ...
                .transpose()?;
            let mut full_text_query = FullTextSearchQuery::new(column, query);
